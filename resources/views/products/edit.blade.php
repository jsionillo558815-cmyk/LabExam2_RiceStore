...
<div class="form-page">
    <div class="form-card">
        <h3>Edit Rice Product</h3>
        <form action="{{ route('products.update', $product) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label>Rice Name</label>
                <input type="text" name="rice_name" value="{{ $product->rice_name }}" required>
            </div>
            <div class="form-group">
                <label>Price per Kilo (₱)</label>
                <input type="number" step="0.01" name="price_per_kilo" value="{{ $product->price_per_kilo }}" required>
            </div>
            <div class="form-group">
                <label>Stock per Kilo</label>
                <input type="number" step="0.01" name="stock_per_kilo" value="{{ $product->stock_per_kilo }}" required>
            </div>
            <div class="form-group">
