    ...
    <div class="form-card">
        ...
        <form action="{{ route('orders.update', $order) }}" method="POST">
                ...
                <select name="products_id" id="product_select" required>
                    ...
                    @foreach($products as $product)
                        <option value="{{ $product->id }}"
                            {{ $order->products_id == $product->id ? 'selected' : '' }}
            ...
            <div class="form-group">
                ...
                <input type="text" id="rice_name" name="rice_name" value="{{ $order->rice_name }}" required readonly>
            </div>
            <div class="form-group">
                <label>Price per Kilo (₱)</label>
                <input type="number" step="0.01" id="price_per_kilo" name="price_per_kilo" value="{{ $order->price_per_kilo }}" required readonly>
            </div>
            <div class="form-group">
                <label>Quantity (kg)</label>
                <input type="number" id="quantity" name="quantity" value="{{ $order->quantity }}" required min="0.1" step="0.1">
            </div>
            <div class="form-group">
                <label>Total Amount (₱)</label>
                <input type="number" step="0.01" id="total_amount" name="total_amount" value="{{ $order->total_amount }}" required readonly>
            </div>
